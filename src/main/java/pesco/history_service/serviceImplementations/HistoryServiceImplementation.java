...
public class HistoryServiceImplementation implements HistoryService{
    ...
    public ResponseEntity<?> createUserCreditHistory(HttpServletRequest httpRequest, String token, String username,
        ...
        userHistory.setTimestamp(Timestamp.from(Instant.now()));
        userHistory.setStatus(TransactionStatus.Success.toString());
        userHistory.setDescription(request.getDescription());

        try {
            historyRepository.save(userHistory);
            return Error.createResponse("Successfully created history", HttpStatus.CREATED,
                    "Successfully Create history");
        } catch (Exception e) {
            return Error.createResponse("Failed to process history.", HttpStatus.BAD_REQUEST,
                    "details " + e.getMessage());
        }
    }
    ...
    @Override
    public List<HistoryDTO> fetchUserHistoryByType(String username, String userId, TransactionType type) {
        long Id = Long.parseLong(userId);
        List<History> depositsHistoryList = historyRepository.findByType(Id, type);

        return depositsHistoryList.stream()
                .map(HistoryDTO::new)
                .collect(Collectors.toList());
    }
